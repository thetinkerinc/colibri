...
onMount(() => {
	...
	container.scrollBy(
		elemBounds.x - containerBounds.x - containerBounds.width / 2 + elemBounds.width / 2,
		elemBounds.y - containerBounds.y - containerBounds.height / 2 + elemBounds.height / 2
	);
});
...
<div class="text-lg">
	A utility component which anchors content to another element. Anchored is meant to be a base from
	which you can build any component which needs to be positioned relative to another. Some examples
	in Colibri are the tooltip (documentation, source code) and the date picker (documentation, source
	code).
	<br /><br />
	...
	to stay visible as long as possible when scrolling or resizing, but will close automatically if it
	lies mostly out of frame.
	<br /><br />
...
<Example code={basic}>
	...
	<Anchored bind:open position="bottom" anchor={elem}>
		<div class="max-w-[300px] rounded border border-gray-300 bg-emerald-200 py-2 px-4 text-center">
