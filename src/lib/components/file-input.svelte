...
{#if $$slots.trigger}
	<span on:click={() => input.click()} on:keyup={() => input.click()}>
		<slot name="trigger" />
	</span>
{:else}
			...
			{#if file.type === 'application/pdf'}
				<div>
					<span class="fa-stack fa-4x h-[1em] w-[1em] leading-[1em]">
						<i class="far fa-file-pdf fa-stack-1x text-red-500" />
						<i class="far fa-file fa-stack-1x" />
					</span>
					<div class="my-1">{file.name}</div>
				</div>
			{:else if showPreview}
				<img
					class="mb-2 block max-w-[350px] rounded border border-gray-300"
					src={preview}
					alt="Preview" />
			{/if}
...
<Modal {open} on:close={handleClose}>
	{#if editing}
		<div class="mx-auto w-[80%] max-w-[600px]">
			<Cropper
				src={image}
				{aspect}
				{outputWidth}
	...
	{:else}
		<div class="flex flex-col items-center gap-3">
			<img
				class="mb-1 block w-[80%] max-w-[350px] rounded border border-gray-300"
				src={preview}
				alt="Preview" />
